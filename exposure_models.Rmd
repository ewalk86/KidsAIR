...
selected_vars <- analysis_data %>% 
  select(area, pm_mean_sampling_period, income_3level, 
         stove_age_3level, home_windows_2level,
         chimney_clean_3level, 
         home_act_door_2level, home_act_windows_2level, home_act_smoke_2level,
         sums_events_sampling_period, mean_temp_sampling_period_5, 
         amb_pm_sampling_period_5, stove_grade_3level, moisture_closest_2level) %>% 
  mutate(area = factor(area, levels = c("AK", "NN", "WMT")),
         log_pm_mean_sampling_period = log(pm_mean_sampling_period)) %>% 
  select(-pm_mean_sampling_period) %>% 
  filter(!is.na(income_3level) & !is.na(home_windows_2level)) %>% 
  filter(area == "WMT") %>% 
  select(-area)
...
imputed_data <- mice(selected_vars, m=5, maxit = 5,
                     method = c("polr", "polr", "logreg", "polr", 
                                "logreg", "logreg", "logreg", "pmm", "pmm",
                                "pmm", "polr", "logreg", "pmm"))
