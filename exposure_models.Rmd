...
# Check % missing data
mean(is.na(selected_vars)) # 10.7% for WMT
...
missing_summary <- complete_case %>% 
  group_by(area) %>% 
  summarize(n = n(),
            mean_var = mean(pm_mean_sampling_period, na.rm = TRUE), 
            sd_var = sd(pm_mean_sampling_period, na.rm = TRUE),
            median_var = median(pm_mean_sampling_period, na.rm = TRUE)) %>% 
  group_by(area) %>% 
  select(area, n, mean_var, sd_var, median_var)
missing_summary

missing_summary <- complete_case %>% 
  group_by(area, wood_collect_2level) %>% 
  summarize(n = n()) %>% 
  group_by(area) %>% 
  mutate(total_n = sum(n),
         perc = n/total_n*100) %>% 
  select(area, wood_collect_2level, n, perc, total_n)
missing_summary
...
imputed_data <- mice(selected_vars, m=10, maxit = 40,
                     method = c("polr", "logreg", "logreg", "logreg", "logreg",
                                "polr", "polr", "logreg", "logreg", "pmm",
                                "pmm", "polr", "logreg", "pmm"))

write_rds(imputed_data, paste0(file_path, "Output/imputed_exposure_data_22may2020.rds"))

imputed_exposure_data <- read_rds(paste0(file_path, "Output/imputed_exposure_data_22may2020.rds"))
...
write_rds(model_ave, paste0(file_path, "Output/imputed_exposure_model_22may2020.rds"))

exposure_model <- read_rds(paste0(file_path, "Output/imputed_exposure_model_22may2020.rds"))
...
model_results <- lm(log_pm_mean_sampling_period ~
                      ...
                      chimney_clean_3level +
                      stove_grade_3level,
                      data = imputed_exposure_data$data)
