...
selected_vars <- analysis_data %>% 
  #filter(area == "WMT") %>% 
  select(pm_mean_sampling_period, income_3level, area, home,
         residents_smoke,
...
the observations drops to 34. While there is only 7.5% missing data, it is spread
...
impute the 7.5% missing data and use that for model selection.
...
# Check % missing data
mean(is.na(selected_vars)) # 7.5% for WMT
