...
author: "Ethan Walker"
date: "Started 19 March 2020, Updated 30 April 2020"
output: pdf_document
---
...
Add some of the variables created below to the full dataset
...
selected_vars <- analysis_data %>% 
  #filter(area == "WMT") %>% 
  select(home, area, pm_mean_sampling_period, income_3level, 
         residents_smoke, home_floors_2level, home_pets_2level,
         chimney_clean_3level, stove_cert, wood_collect_2level,
         moisture_closest_2level, sums_events_sampling_period,
         mean_temp_sampling_period_5, amb_pm_sampling_period_5,
         home_act_smoke_2level, home_act_door_2level, stove_grade_3level) %>% 
...
the observations drops to 30. While there is only 10% missing data, it is spread
...
impute the 10% missing data and use that for model selection.
...
missing_test <- selected_vars %>% 
  mutate(missing_var = if_else(is.na(home_act_door_2level), "missing yes", "missing no")) %>% 
...
missing_summary <- complete_case %>% 
  group_by(area) %>% 
  summarize(n = n(),
            mean_pm = mean(mean_temp_sampling_period_5, na.rm = TRUE), 
            sd_pm = sd(mean_temp_sampling_period_5, na.rm = TRUE),
            median_pm = median(mean_temp_sampling_period_5, na.rm = TRUE)) %>% 
  group_by(area) %>% 
  select(area, n, mean_pm, sd_pm, median_pm)
missing_summary
...
imputed_data <- mice(selected_vars, m=10, maxit = 5,
                     method = c("polr", "logreg", "logreg", "logreg", "polr",
                                "polr", "logreg", "logreg", "pmm", "pmm",
                                "pmm", "logreg", "logreg", "polr", "pmm"))
