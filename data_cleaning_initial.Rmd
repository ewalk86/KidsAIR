...
author: "Ethan Walker"
date: "Started 19 Nov 2019, Updated 4 Dec 2019"
output: pdf_document
---
...
```{r}
##### Read in dusttrack .txt files (from WMT location) #####
...
# Format combined file from above; combine with other WMT/PM data below
wmt_pm_txt <- initial_data %>% 
  mutate(date = mdy(`MM/dd/yyyy`),
         time = hms(`hh:mm:ss`),
         pm = as.numeric(`mg/m^3`),
         area = "WMT") %>% 
  select(home_winter_id, date, time, pm, area)
  

##### Next, load other files from all locations #####
