...
# Load, format, and save KidsAIR Dusttrak data
```{r}
...
```{r}
kids_pm_full <- read_rds("Output/kids_pm_full.rds") 
  
kids_pm_check <- kids_pm_full %>% 
...
dt_home <- rbind(ak_dt_home, nn_dt_home, wmt_dt_home) %>% 
  ...
  left_join(kids_pm_check, by = c("area", "home_winter_id")) 
...
kids_pm_new <- kids_pm_full %>%  
  #filter(area == "AK") %>% 
  # apply correction factor to pm
  mutate(pm = pm/1.65) %>% 
  ...
  mutate(pm_total_observations = n(),
         ...
         # mean pm based on 24hr periods in the sampling interval
         pm_24hr_mean_interval = pm_total_summed/pm_sample_interval,
         # mean pm based on 24hr periods of total observations
         pm_24hr_mean_obs = pm_total_summed/(pm_total_observations/1440),
         # rolling 24hr mean pm
