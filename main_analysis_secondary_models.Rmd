...
er_results <- glmer.nb(lrti_events_total ~ pm_mean_sampling_period_iqr + child_age + 
                          ...
                          (1 | cohort:area) + offset(log_person_time_at_risk), 
                          data = analysis_data)
...
```{r, echo=TRUE}
# Run primary model and print results
logreg_results <- glmer(lrti_events_di_total ~ treatment_assigned + child_age + person_time_at_risk +
                        (1 | home:cohort:area),
...
# Run primary model and print results
logreg_results <- glmer(lrti_events_di_total ~ pm_mean_sampling_period_iqr + child_age + 
                        education_3level + residents_smoke + 
                        (1 | cohort:area),
                        data = analysis_data, family = binomial, nAGQ = 20)
#write_rds(logreg_results, paste0(file_path, "Output/logreg_results.rds"))
