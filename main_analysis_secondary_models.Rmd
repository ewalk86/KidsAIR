...
author: "Ethan Walker"
date: "Started 28 Oct 2020, Updated 9 Nov 2020"
output: pdf_document
---
...
itt_results_combined <- rbind(itt_tidy, itt_winter1_tidy, itt_winter2_tidy,
...
itt_plot_estimates <- itt_results_combined %>%
  mutate(model = factor(model,
                        levels = c("ITT Primary", "ITT Winter 1", "ITT Winter 2", 
...
er_plot_estimates <- er_results_combined %>%
  ...
  geom_hline(yintercept = 0) +
  theme_bw() +  
  labs(y = "LRTI cases per IQR increase n\ in PM2.5") +
  labs(x = "", group = "", shape = "") +
  theme(title = element_text(size = 16), 
          axis.text.x = element_text(size = 16, colour = "black", angle = 45,
                                     hjust = 1, vjust = .8),
