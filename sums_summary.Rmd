...
author: "Ethan Walker"
date: "Started 9 Jan 2020, Updated 28 Jan 2020"
output: pdf_document
---
...
pm_homes_placebo <- pm_clean %>% 
  ...
  mutate(pm_home = "true") %>% 
  select(area, home, home_winter_id, pm_home, pm_datetime_new) %>% 
  separate(pm_datetime_new, c("sample_date", "sample_time"), sep = " ", remove = TRUE) %>% 
...
sums_filtered_date <- sums_clean %>% 
  ...
  # change to correct joining data of interest
  left_join(pm_homes_placebo, by = c("area", "home", "home_winter_id", "sample_date")) %>% 
  filter(pm_home == "true") %>% 
  filter(winter_id == 1) %>% 
  ungroup() %>% 
  arrange(area, home)
...
thresholds_function(sums_filtered_date, 35, 1.5)
thresholds_function(sums_filtered_date, 40, 1.5)
thresholds_function(sums_filtered_date, 45, 1.5)
thresholds_function(sums_filtered_date, 50, 1.5)
thresholds_function(sums_filtered_date, 55, 1.5)
```
...
thresholds_percent(sums_filtered_date, 35)
thresholds_percent(sums_filtered_date, 40)
thresholds_percent(sums_filtered_date, 45)
thresholds_percent(sums_filtered_date, 50)
thresholds_percent(sums_filtered_date, 55)
```
