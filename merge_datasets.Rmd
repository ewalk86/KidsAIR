...
author: "Ethan Walker"
date: "Started 24 March 2020, Updated 14 April 2020"
output: pdf_document
---
...
pm_clean <- 
  read_rds(paste0(output_path, "Output/pm_clean.rds"))
sums_clean <- 
  read_rds(paste0(output_path, "Output/sums_clean.rds"))

kids_linked_ids <- 
  read_rds(paste0(output_path, "Output/kids_linked_ids.rds"))
demographics_clean <- 
  read_rds(paste0(output_path, "Output/demographics_clean.rds"))
moisture_clean <- 
  read_rds(paste0(output_path, "Output/moisture_clean.rds"))
indoor_temp_rh_clean <- 
  read_rds(paste0(output_path, "Output/indoor_temp_rh_clean.rds"))
stove_use_clean <- 
  read_rds(paste0(output_path, "Output/stove_use_clean.rds"))
stove_grades_clean <- 
  read_rds(paste0(output_path, "Output/stove_grades_clean.rds"))
home_char_clean <- 
  read_rds(paste0(output_path, "Output/home_char_clean.rds"))
home_act_clean <- 
  read_rds(paste0(output_path, "Output/home_act_clean.rds"))
```
...
pm_join <- pm_clean %>% 
  ...
  select(area, home, home_winter_id, winter_id, treatment, dtid, day_of_week, 
         ...
         pm_spike_2000, pm_spike_5000) %>% 
  group_by(area, home) %>% 
  distinct(sample_date, .keep_all = TRUE) %>% 
  ungroup() %>% 
  arrange(area, home, sample_date)

#write_rds(pm_join, paste0(output_path, "Output/pm_join.rds"))
pm_join <- read_rds(paste0(output_path, "Output/pm_join.rds"))
...
#write_rds(sums_join, paste0(output_path, "Output/sums_join.rds"))
sums_join <- read_rds(paste0(output_path, "Output/sums_join.rds"))
...
#write_rds(pm_sums_joined1, paste0(output_path, "Output/pm_sums_joined1.rds"))
pm_sums_joined1 <- read_rds(paste0(output_path, "Output/pm_sums_joined1.rds"))
...
#write_rds(analysis_data7, paste0(output_path, "Output/exposure_analysis_data_long.rds"))
#write_csv(analysis_data7, paste0(output_path, "Output/exposure_analysis_data_long.csv"))


exposure_analysis_data_long <- 
  read_rds(paste0(output_path, "Output/exposure_analysis_data_long.rds"))
...
kids_amb_pm_data <- 
  read_rds(paste0(output_path, "Output/kids_amb_pm_data_new.rds")) %>% 
  mutate(sample_date = amb_pm_date) %>% 
  distinct(zip, .keep_all = TRUE)

kids_amb_temp_data <- 
  read_rds(paste0(output_path, "Output/kids_amb_temp_data_new.rds")) %>% 
  mutate(sample_date = temp_date) %>% 
  mutate(sample_date = as.character(sample_date)) %>% 
  distinct(zip, .keep_all = TRUE)

wmt_zipcodes <- read_csv(paste0(output_path, "Input/WMT/wmt_zipcodes.csv")) %>% 
  rename_all(tolower)

nn_zipcodes <- read_csv(paste0(output_path, "Input/NN/nn_zipcodes.csv")) %>% 
  rename_all(tolower) 

ak_zipcodes <- read_csv(paste0(output_path, "Input/AK/ak_zipcodes.csv")) %>% 
  rename_all(tolower) 

kids_zipcodes <- rbind(wmt_zipcodes, nn_zipcodes, ak_zipcodes) %>% 
  mutate(home = gsub(home, pattern = "_", replacement = ""),
         home = gsub(home, pattern = " ", replacement = "")) %>% 
  left_join(kids_amb_pm_data, by = "zip") %>% 
  select(home, zip, lat_pm, long_pm) %>% 
  left_join(kids_amb_temp_data, by = "zip") %>% 
  select(home, zip, lat_zip, long_zip, lat_pm, long_pm, lat_temp, long_temp)

kids_amb_pm_data <- 
  read_rds(paste0(output_path, "Output/kids_amb_pm_data_new.rds")) %>% 
  mutate(sample_date = amb_pm_date) %>% 
  select(-lat_zip, -long_zip, -lat_pm, -long_pm)

kids_amb_temp_data <- 
  read_rds(paste0(output_path, "Output/kids_amb_temp_data_new.rds")) %>% 
  mutate(sample_date = temp_date) %>% 
  mutate(sample_date = as.character(sample_date)) %>% 
  select(-lat_zip, -long_zip, -lat_temp, -long_temp)
...
exposure_analysis_data_long_new <- exposure_analysis_data_long %>%
  ...
  mutate(total_residents = if_else(home == "WMA438", "6", total_residents),
         ...
         income_3level = factor(income_3level, levels = c("Less than $20,000", 
                                            ...
                                            "$50,000+"))) %>% 
  ungroup() %>% 
  left_join(kids_zipcodes, by = "home") %>% 
  left_join(kids_amb_pm_data, by = c("area", "zip", "sample_date")) %>% 
  left_join(kids_amb_temp_data, by = c("area", "zip", "sample_date")) %>%
  arrange(area, home, home_winter_id, sample_date)



#write_rds(exposure_analysis_data_long_new, paste0(output_path, "Output/exposure_analysis_data_long_new.rds"))
#write_csv(exposure_analysis_data_long_new, paste0(output_path, "Output/exposure_analysis_data_long_new.csv"))
```
...
exposure_analysis_data_medium <- exposure_analysis_data_long_new %>% 
  ...
  distinct(sampling_day, .keep_all = TRUE) %>% 
  select(area, home, home_winter_id, winter_id, treatment, sampling_day,
         child_id_char, burn_level:mean_temp_roll_4day) %>% 
...
#write_rds(exposure_analysis_data_medium, paste0(output_path, "Output/exposure_analysis_data_medium.rds"))
#write_csv(exposure_analysis_data_medium, paste0(output_path, "Output/exposure_analysis_data_medium.csv"))
```
...
#write_rds(exposure_analysis_data_short, paste0(output_path, "Output/exposure_analysis_data_short.rds"))
#write_csv(exposure_analysis_data_short, paste0(output_path, "Output/exposure_analysis_data_short.csv"))
```
...
#write_rds(pm_sums_joined3, paste0(output_path, "Output/sums_rolling_pm_data.rds"))
#write_csv(pm_sums_joined3, paste0(output_path, "Output/sums_rolling_pm_data.csv"))
```
