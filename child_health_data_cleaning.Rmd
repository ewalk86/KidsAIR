...
author: "Ethan Walker"
date: "Started 27 Feb 2020, Updated 20 April 2020"
output: pdf_document 
---
...
demographics_clean <- 
  read_rds(paste0(file_path, "Output/demographics_clean.rds"))
...
```{r}
wmt_athomelog_initial <- read_xlsx(paste0(file_path, "Input/WMT/wmt_athomelog_20200227.xlsx")) %>% 
...
nn_athomelog_initial <- read_xlsx(paste0(file_path, "Input/NN/nn_athomelog_20200416.xlsx")) %>% 
  mutate(home_winter_id = as.factor(HomeWinterID),
         child_id_num = as.factor(ChildID),
         area = "NN",
         at_home_day = as.factor(Day),
...
ak_athomelog_initial <- read_xlsx(paste0(file_path, "Input/AK/ak_athomelog_20200227.xlsx")) %>% 
  mutate(home_winter_id = as.factor(HomeWinterID),
         child_id_num = as.factor(ChildID),
         area = "AK",
         at_home_day = as.factor(Day),
...
nn_medicalvisit_initial <- read_xlsx(paste0(file_path, "Input/NN/nn_medicalvisit_20200416.xlsx")) %>%
  mutate(HRPulseOx = if_else(HRPulseOx < 50, -999, HRPulseOx),
         HR = if_else(HR < 50, -999, HR),
         SP02 = if_else(SP02 < 70, -999, SP02)) %>% 
  replace_with_na_all(condition = ~.x %in% c(-999, -9999, 0)) %>% 
...
ak_medicalvisit_initial <- read_xlsx(paste0(file_path, "Input/AK/ak_medicalvisit_20200416.xlsx")) %>% 
  mutate(HRPulseOx = if_else(HRPulseOx < 50, -999, HRPulseOx),
         HR = if_else(HR < 50, -999, HR),
         SP02 = if_else(SP02 < 70, -999, SP02)) %>% 
  replace_with_na_all(condition = ~.x %in% c(-999, -9999, 0)) %>% 
...
# Load and merge files from different locations
```{r}
wmt_birthinfancy_initial <- read_xlsx(paste0(file_path, "Input/WMT/wmt_birthinfancy_20200227.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
         child_id_num = as.character(ChildID),
         area = "WMT",
...
nn_birthinfancy_initial <- read_xlsx(paste0(file_path, "Input/NN/nn_birthinfancy_20200416.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
         child_id_num = as.character(ChildID),
         area = "NN",
         gender = as.character(Gender),
...
ak_birthinfancy_initial <- read_xlsx(paste0(file_path, "Input/AK/ak_birthinfancy_20200416.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
         child_id_num = as.character(ChildID),
         area = "AK",
         gender = as.character(Gender),
...
wmt_childhomewinter_initial <- 
  read_xlsx(paste0(file_path, "Input/WMT/wmt_childhomewinter_20200227.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
...
child_home_winter <- read_rds(paste0(file_path, "Output/child_home_winter.rds"))
...
```{r}
wmt_csq_initial <- 
  read_xlsx(paste0(file_path, "Input/WMT/wmt_csq_20200227.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
         child_id_num = as.character(ChildID),
         area = "WMT",
         csq_date = ymd(Date),
...
nn_csq_initial <- 
  read_xlsx(paste0(file_path, "Input/NN/nn_csq_20200416.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
         child_id_num = as.character(ChildID),
         area = "NN",
         csq_date = ymd(Date),
         ...
         csq_cough_current = as.character(CoughingStill),
         csq_wheeze_current = as.character(WheezingStill),
         csq_comments = as.character(`REPLACE(REPLACE(Comments, '\\n', ''), '\\r', '')`)) %>% 
  replace_with_na_all(condition = ~.x %in% c(-999, -99, -99.9, 999, 0)) %>% 
  select(home_winter_id:csq_comments)


ak_csq_initial <- 
  read_xlsx(paste0(file_path, "Input/AK/ak_csq_20200416.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
         child_id_num = as.character(ChildID),
         area = "AK",
         csq_date = ymd(Date),
...
wmt_medicalrecords_initial <- 
  read_xlsx(paste0(file_path, "Input/WMT/wmt_medicalrecords_20200227.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
         child_id_num = as.character(ChildID),
         area = "WMT",
...
nn_medicalrecords_initial <- 
  read_xlsx(paste0(file_path, "Input/NN/nn_medicalrecords_20200416.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
         child_id_num = as.character(ChildID),
         area = "NN",
         visit_date = ymd(VisitDate),
...
ak_medicalrecords_initial <- 
  read_xlsx(paste0(file_path, "Input/AK/ak_medicalrecords_20200416.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
         child_id_num = as.character(ChildID),
         area = "AK",
         visit_date = ymd(VisitDate),
...
kids_medical_records <- read_rds(paste0(file_path, "Output/kids_medical_records.rds"))
```
