...
pm_guidelines <- exposure_analysis_data_short %>% 
  filter(winter_id == 1 & treatment == "Placebo") %>% 
  filter(home != "WMA300") %>% 
  filter(pm_mean_sampling_period > 35) %>% 
...
pm_check <- exposure_analysis_data_short %>% 
  filter(winter_id == 1 & treatment == "Placebo") %>% 
  filter(home != "WMA300") %>% 
  distinct(home, .keep_all = TRUE) %>% 
...
pm_stove_grade <- exposure_analysis_data_short %>%
  select(area, home, pm_mean_sampling_period, stove_grade_3level, 
         treatment, winter_id, home_winter_id) %>% 
  filter(winter_id == 1 & treatment == "Placebo") %>% 
  filter(home != "WMA300") %>% 
  distinct(home, .keep_all = TRUE) %>% 
...
```{r, fig.height=, fig.width=8}
boxplots_kids_pm <- exposure_analysis_data_short %>% 
  filter(winter_id == 1 & treatment == "Placebo") %>% 
  filter(home != "WMA300") %>% 
...
ggsave("boxplots_kids_pm.jpg", height = 7, width = 8)
```
...
```{r}
# check PM spikes
spikes_data <- exposure_analysis_data_medium %>% 
  filter(winter_id == 1 & treatment == "Placebo") %>% 
  filter(home != "WMA300") %>% 
