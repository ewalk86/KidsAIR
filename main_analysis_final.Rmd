...
# Primary analysis dataset - load and format a few variables
analysis_data <- read_rds(paste0(file_path, 
  ...
  mutate(pm_mean_sampling_period_iqr = pm_mean_sampling_period/25,
         ...
         pm_mean_at_home_sampling_period = pm_at_home_sampling_period,
  ...
  #filter(!is.na(home_sqft_2level))
  #filter(area == "WMT")
  #filter(gender == "Female")
  #filter(home_sqft_2level == "1500+")


# Dataset for sensitivity analysis - replace "outlier" for person time with mean value
...
# Dataset for sensitivity analysis - filter for Winter 1 data only
...
# Dataset for sensitivity analysis - filter for Winter 2 data only
...
# Dataset for sensitivity analysis - filter out AK data
...
itt_results_int <<- glmer(lrti_events_di_total ~ treatment_assigned*int_var + 
                          child_age + 
                          person_time_at_risk + (1 | home:cohort:area),
...
# Run function with selected interaction term
# Use next few lines to help fill in Tables for publication
int_function(analysis_data, "flu_shot")
...
obs <- analysis_data %>% 
  filter(!is.na(lrti_events_di_total)) %>% 
  filter(!is.na(child_age)) %>%
  filter(!is.na(area)) %>% 
  filter(!is.na(cohort)) %>% 
  filter(!is.na(treatment_assigned)) %>% 
  filter(!is.na(residents_smoke)) %>%
  group_by(residents_smoke, treatment_assigned) %>% 
  count()
obs

# Try child_age, gender, home_floors_2level, home_sqft_2level, residents_smoke, flu_shot
```


```{r, eval=FALSE, include=FALSE} 
# Run and save model results for various sensitivity analyses 
...
itt_fluvax_results <- glmer(lrti_events_di_total ~ treatment_assigned + 
                     child_age + person_time_at_risk + flu_shot + 
                     (1 | home:cohort:area),
                     data = analysis_data, family = binomial, nAGQ = 10)
write_rds(itt_fluvax_results, paste0(file_path, "Output/itt_fluvax_results.rds"))
...
itt_area_results <- glmer(lrti_events_di_total ~ treatment_assigned + 
                     child_age + area + person_time_at_risk + 
                     (1 | home:cohort),
                     data = analysis_data, family = binomial, nAGQ = 10)
write_rds(itt_area_results, paste0(file_path, "Output/itt_area_results.rds"))


itt_offset_results <- glmer(lrti_events_di_total ~ treatment_assigned + 
                     child_age + 
                     (1 | home:cohort:area),
                     data = analysis_data, family = binomial, nAGQ = 10)
write_rds(itt_offset_results, paste0(file_path, "Output/itt_offset_results.rds"))
...
itt_fluvax_results <- read_rds(paste0(file_path, "Output/itt_fluvax_results.rds"))
itt_home_results <- read_rds(paste0(file_path, "Output/itt_home_results.rds"))
itt_area_results <- read_rds(paste0(file_path, "Output/itt_area_results.rds"))
itt_offset_results <- read_rds(paste0(file_path, "Output/itt_offset_results.rds"))
...
itt_fluvax_tidy <- results_function(itt_fluvax_results, "ITT + flu vax")
itt_home_tidy <- results_function(itt_home_results, "ITT no Home")
itt_area_tidy <- results_function(itt_area_results, "ITT area fixed")
itt_offset_tidy <- results_function(itt_offset_results, "ITT no offset")
...
itt_results_combined <- rbind(itt_tidy, itt_winter1_tidy, itt_noak_tidy,
                              ...
                              itt_ambpm_tidy, itt_ambtemp_tidy, itt_fluvax_tidy, 
                              itt_area_tidy, itt_offset_tidy, itt_home_tidy)
...
itt_plot_estimates <- itt_results_combined %>%
  mutate(model = factor(model,
                        levels = c("ITT Primary", "ITT Winter 1", "ITT no AK",
                                   ...
                                   "ITT + amb PM", "ITT + amb temp", "ITT + flu vax", 
                                   "ITT area fixed", "ITT no offset", "ITT no Home"))) %>% 
  ...
  labs(y = "Odds of LRTI compared to Control") +
  labs(x = "", group = "", shape = "") +
  theme(title = element_text(size = 16), 
          axis.text.x = element_text(size = 16, colour = "black", angle = 45,
                                     hjust = 1, vjust = .8),
...
ITT Primary = Logistic regression mixed model, as specified in main results  
ITT Winter 1 = Primary model with Winter 1 data only  
ITT no AK = Primary model without homes from Alaska (different placebo group)  
...
ITT + flu vax = Primary model plus caregiver-reported influenza vaccination status of child  
ITT area fixed = Primary model with area as a fixed term instead of a random term
ITT no offset = Primary model without fixed offset term for person-time of follow-up
ITT no Home = Primary model without the random nested term for household/home
...
```{r}
# select distinct homes within datasets
winter_1_data_pm <- analysis_data_winter1 %>% 
  distinct(area, home, .keep_all = T)

winter_1_data_pm_noak <- winter_1_data_pm %>% 
  distinct(area, home, .keep_all = T) %>% 
  filter(area != "AK")
```
...
itt_pm_results <- lmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                       (1 | cohort:area), data = winter_1_data_pm)
...
# Run function with selected interaction term
# Use next few lines to help fill in Tables for publication
int_function(winter_1_data_pm, "stove_grade_3level")
...
obs <- winter_1_data_pm %>% 
  ...
  filter(!is.na(cohort)) %>% 
  filter(!is.na(treatment_assigned)) %>% 
  filter(!is.na(stove_grade_3level)) %>%
  group_by(stove_grade_3level, treatment_assigned) %>% 
  count()
obs


# Try: home_floors_2level, home_sqft_2level, education_3level, child_age
# home_year_built_2level, stove_age_3level, stove_age_di, stove_grade_3level, chimney_clean_3level,
# wood_collect_2level, wood_collect_method_2level, burn_level_3level, residents_smoke

((exp(-0.07214)-1)*100) # formula for exponentiating results (percent difference)
```
...
itt_pm_noak_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                       (1 | cohort:area), data = winter_1_data_pm_noak)
summary(itt_pm_noak_results)
write_rds(itt_pm_noak_results, paste0(file_path, "Output/itt_pm_noak_results.rds"))


itt_pm_at_home_results <- glmer(log(pm_mean_at_home_sampling_period) ~ treatment_assigned + 
                               (1 | cohort:area), 
                               data = winter_1_data_pm)
write_rds(itt_pm_at_home_results, paste0(file_path, "Output/itt_pm_at_home_results.rds"))


itt_pm_income_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                               (1 | cohort:area) + 
                               income_3level, data = winter_1_data_pm)
write_rds(itt_pm_income_results, paste0(file_path, "Output/itt_pm_income_results.rds"))


itt_pm_sqft_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                               (1 | cohort:area) + 
                               home_sqft_2level, data = winter_1_data_pm)
write_rds(itt_pm_sqft_results, paste0(file_path, "Output/itt_pm_sqft_results.rds"))
...
itt_pm_filter_comp_results <- glmer(log(pm_mean_sampling_period) ~ filter_compliance + 
                                  (1 | cohort:area), 
                                  data = winter_1_data_pm)
summary(itt_pm_filter_comp_results)
...
itt_pm_smoke_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                              (1 | cohort:area) + 
                              residents_smoke, data = winter_1_data_pm)
...
itt_pm_burn_level_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                                   (1 | cohort:area) + 
                                   burn_level_3level, data = winter_1_data_pm)
...
itt_pm_smoking_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                                   (1 | cohort:area) + 
                                   home_act_smoking_sum, data = winter_1_data_pm)
...
itt_pm_windows_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                                   (1 | cohort:area) + 
                                   home_act_windows_sum, data = winter_1_data_pm)
...
itt_pm_door_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                                   (1 | cohort:area) + 
                                   home_act_door_sum, data = winter_1_data_pm)
...
itt_pm_sweep_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                                   (1 | cohort:area) + 
                                   home_act_sweep_sum, data = winter_1_data_pm)
...
itt_pm_ambpm_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                                   (1 | cohort:area) + 
                                   amb_pm_lrti, data = winter_1_data_pm)
...
itt_pm_ambtemp_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
                                   (1 | cohort:area) + 
                                   amb_temp_lrti, data = winter_1_data_pm)
...
itt_pm_at_home_results <- read_rds(paste0(file_path, "Output/itt_pm_at_home_results.rds"))
...
itt_pm_sqft_results <- read_rds(paste0(file_path, "Output/itt_pm_sqft_results.rds"))
...
itt_pm_at_home_tidy <- results_function(itt_pm_at_home_results, "ITT PM at-home")
...
itt_pm_sqft_tidy <- results_function(itt_pm_sqft_results, "ITT + home size")
...
# Combine cleaned results
itt_pm_results_combined <- rbind(itt_pm_tidy, itt_pm_at_home_tidy, itt_pm_noak_tidy, 
                                 itt_pm_income_tidy, itt_pm_sqft_tidy, 
...
itt_pm_plot_estimates <- itt_pm_results_combined %>%
  mutate(model = factor(model,
                        levels = c("ITT Primary", "ITT PM at-home", "ITT no AK", 
                                   "ITT + income", "ITT + home size",
                                   "ITT + smoke hx", "ITT + burn level", "ITT + smoke act",
                                   "ITT + window act", "ITT + door act", "ITT + sweep act",
  ...
  geom_hline(yintercept = 0) +
  theme_bw() +  
  ggtitle(label = "Results for ITT framework, PM2.5 outcome") +
  labs(y = "% difference in PM2.5 \n compared to Control") +
  labs(x = "", group = "", shape = "") +
  theme(title = element_text(size = 16), 
          axis.text.x = element_text(size = 16, colour = "black", angle = 45,
...
ITT PM at-home = Primary model using PM when child was at home  
...
er_results <- glmer(lrti_events_di_total ~ pm_mean_sampling_period_iqr + 
                          child_age + gender + 
                          race_parent + gender_parent +
                          income_3level + education_3level + 
                          amb_temp_lrti + 
                          #amb_pm_lrti + 
                          residents_smoke +
                          person_time_at_risk +
...
#nobs(er_results)
#table(model.frame(er_results)$residents_smoke)
...
# Run primary model and print results
er_results_int <<- glmer(lrti_events_di_total ~ pm_mean_sampling_period_iqr*int_var + 
                          child_age + 
                          gender + 
                          #residents_smoke + 
                          race_parent + gender_parent +
                          income_3level + education_3level + 
                          amb_temp_lrti + amb_pm_lrti + 
                          person_time_at_risk +
                          (1 | home:cohort:area), 
                          data = new_model_data, family = binomial, nAGQ = 10,
                          control = glmerControl(optimizer = "bobyqa"))
...
# Try child_age, gender, residents_smoke
```
...
er_results_at_home <- er_results <- glmer(lrti_events_di_total ~ pm_mean_at_home_sampling_period_iqr + 
                          child_age + gender + 
                          race_parent + gender_parent +
                          income_3level + education_3level + residents_smoke +
                          amb_temp_lrti + amb_pm_lrti + 
                          person_time_at_risk +
                          (1 | home:cohort:area), 
                          data = analysis_data, family = binomial, nAGQ = 10,
                          control = glmerControl(optimizer = "bobyqa"))
write_rds(er_results_at_home, paste0(file_path, "Output/er_results_at_home.rds"))


er_winter1_results <- er_results <- glmer(lrti_events_di_winter ~ pm_mean_sampling_period_iqr + 
                          child_age + gender + 
                          race_parent + gender_parent +
                          income_3level + education_3level + residents_smoke +
                          amb_temp_lrti + amb_pm_lrti + 
...
er_burn_level_results <- er_results <- glmer(lrti_events_di_total ~ pm_mean_sampling_period_iqr + 
                          burn_level_3level + 
                          child_age + gender + 
                          race_parent + gender_parent +
                          income_3level + education_3level + residents_smoke +
...
er_home_size_results <- er_results <- glmer(lrti_events_di_total ~ pm_mean_sampling_period_iqr + 
                          home_sqft_2level + 
                          child_age + gender + 
                          race_parent + gender_parent +
                          income_3level + education_3level + residents_smoke +
                          ...
                          data = analysis_data, family = binomial, nAGQ = 10,
                          control = glmerControl(optimizer = "bobyqa"))
write_rds(er_home_size_results, paste0(file_path, "Output/er_home_size_results.rds"))


er_stove_grade_results <- er_results <- glmer(lrti_events_di_total ~ pm_mean_sampling_period_iqr + 
                          stove_grade_3level + 
                          child_age + gender + 
                          race_parent + gender_parent +
                          income_3level + education_3level + residents_smoke +
                          ...
                          data = analysis_data, family = binomial, nAGQ = 10,
                          control = glmerControl(optimizer = "bobyqa"))
write_rds(er_stove_grade_results, paste0(file_path, "Output/er_stove_grade_results.rds"))


er_stove_age_results <- er_results <- glmer(lrti_events_di_total ~ pm_mean_sampling_period_iqr + 
                          stove_age_3level + 
                          child_age + gender + 
                          race_parent + gender_parent +
                          income_3level + education_3level + residents_smoke +
                          ...
                          data = analysis_data, family = binomial, nAGQ = 10,
                          control = glmerControl(optimizer = "bobyqa"))
write_rds(er_stove_age_results, paste0(file_path, "Output/er_stove_age_results.rds"))
```
...
er_results_at_home <- read_rds(paste0(file_path, "Output/er_results_at_home.rds"))
...
er_home_size_results <- read_rds(paste0(file_path, "Output/er_home_size_results.rds"))
er_stove_grade_results <- read_rds(paste0(file_path, "Output/er_stove_grade_results.rds"))
er_stove_age_results <- read_rds(paste0(file_path, "Output/er_stove_age_results.rds"))

nobs(er_stove_age_results)
...
er_at_home_tidy <- results_function(er_results_at_home, "ER PM at-home")
er_winter1_tidy <- results_function(er_winter1_results, "ER Winter 1 only")
er_burn_level_tidy <- results_function(er_burn_level_results, "ER + burn level")
er_home_size_tidy <- results_function(er_home_size_results, "ER + home size")
er_stove_grade_tidy <- results_function(er_stove_grade_results, "ER + stove grade")
er_stove_age_tidy <- results_function(er_stove_age_results, "ER + stove age")


# Combine cleaned results
er_results_combined <- rbind(er_tidy, er_at_home_tidy, er_winter1_tidy, er_burn_level_tidy, 
                              er_home_size_tidy, er_stove_grade_tidy, er_stove_age_tidy)


# Plot results
er_plot_estimates <- er_results_combined %>%
  mutate(model = factor(model,
                        levels = c("ER Primary", "ER PM at-home", "ER Winter 1 only", 
                                   "ER + burn level", "ER + home size", 
                                   "ER + stove grade", "ER + stove age"))) %>% 
  ggplot(aes(group = term, shape = term)) +
  geom_point(aes(x=model, y=estimate), 
             position = position_dodge(width = 0.5), size = 4) +
  geom_errorbar(aes(x=model, ymin=conf.low, ymax=conf.high), 
...
ER PM at-home = Primary model using PM from when the child was at home  
ER Winter 1 only = Primary model using data only from Winter 1  
ER + burn level = Primary model plus self-reported burn-level compared to typical  
ER + home size = Primary model plus home size (sqaure meters)  
ER + stove grade = Primary model plus assigned stove grade  
ER + stove age = Primary model plus caregiver reported stove age  


```{r, eval=FALSE, include=FALSE}
################ Code to plot/check individual models and various results ##############
...
lrti_summary_table <- analysis_data_winter2 %>% 
  #filter(!is.na(lrti_events_di_winter) & !is.na(pm_mean_sampling_period) &
   #        !is.na(child_age) & !is.na(education_3level) & !is.na(residents_smoke)) %>% 
  #group_by(treatment_assigned) %>% 
  mutate(lrti_events_num = as.numeric(lrti_events_di_winter),
...
person_time_summary <- analysis_data %>% 
  group_by(treatment_assigned) %>% 
  summarize(mean_var = mean(person_time_at_risk), 
            sd_var = sd(person_time_at_risk), 
            n_var = n(), 
            min_var = min(person_time_at_risk), 
            p25 = quantile(person_time_at_risk, probs = 0.25),
            med_var = median(person_time_at_risk), 
            p75 = quantile(person_time_at_risk, probs = 0.75), 
            max_var = max(person_time_at_risk))
person_time_summary
```
