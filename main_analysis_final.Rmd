...
int_function(analysis_data, "home_sqft_2level")
n_table
save_contrasts
pm_summary_table
lrti_summary_table
...
int_function(analysis_data_winter1, "stove_grade_3level")
n_table
save_contrasts
pm_summary_table
lrti_summary_table
...
obs <- analysis_data_winter1 %>% 
  ...
  group_by(home_sqft_2level, treatment_assigned) %>% 
  count()
obs
...
# Combine cleaned results
itt_pm_results_combined <- rbind(itt_pm_tidy, itt_pm_at_home_tidy, itt_pm_noak_tidy, itt_pm_income_tidy, 
...
itt_pm_plot_estimates <- itt_pm_results_combined %>%
  mutate(model = factor(model,
                        levels = c("ITT Primary", "ITT PM at-home", "ITT no AK", "ITT + income", 
                                   "ITT + smoke hx", "ITT + burn level", "ITT + smoke act",
                                   "ITT + window act", "ITT + door act", "ITT + sweep act",
                                   "ITT + amb PM", "ITT + amb temp"))) %>% 
  ggplot(aes(group = term, shape = term)) +
...
int_function(analysis_data, "child_age")
save_contrasts
pm_summary_table
lrti_summary_table
save_summary
