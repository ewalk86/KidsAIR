...
# Run primary model and print results
itt_results_int <- glmer(lrti_events_di_total ~ treatment_assigned*residents_smoke + 
                     child_age + 
...
int_emmeans <- emmeans(itt_results_int, pairwise ~ treatment_assigned | residents_smoke)
...
itt_noak_results <- glmer(lrti_events_di_total ~ treatment_assigned + 
                     child_age + person_time_at_risk +
                     (1 | home:cohort:area),
                     data = analysis_data_noak, family = binomial, nAGQ = 20)
...
# Run primary model and print results
itt_pm_results_int <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned*int_var + 
                              child_age + 
...
int_emmeans <- emmeans(itt_pm_results_int, pairwise ~ treatment_assigned | int_var)
#summary(int_emmeans, conf.int = TRUE)
save_results <- confint(int_emmeans)
save_results <<- confint(int_emmeans$contrasts)
#save_results

plot_results <- as.data.frame(save_results$contrasts) %>% 
...
int_function(analysis_data_winter1, "home_sqft_2level")
...
# Combine cleaned results
itt_pm_results_combined <- rbind(itt_pm_tidy, itt_pm_noak_tidy, itt_pm_income_tidy, itt_pm_filter_comp_tidy, 
...
itt_pm_plot_estimates <- itt_pm_results_combined %>%
  mutate(model = factor(model,
                        levels = c("ITT Primary", "ITT no AK", "ITT + income", "ITT + filt comp", 
                                   "ITT + smoke hx", "ITT + burn level", "ITT + smoke act",
                                   "ITT + window act", "ITT + door act", "ITT + sweep act",
                                   "ITT + amb PM", "ITT + amb temp"))) %>% 
  ggplot(aes(group = term, shape = term)) +
...
```{r, eval=FALSE, include=FALSE}
# Effect modification/interaction analysis
# Try education_3level, child_age, chimney_clean_3level, wood_collect_2level, residents_smoke

# Run primary model and print results
er_results_int <- glmer.nb(lrti_events_winter ~ pm_mean_sampling_period_iqr*chimney_clean_3level + 
                          child_age + 
                          education_3level + 
                          residents_smoke + 
                          (1 | home:cohort:area) + offset(log_person_time_at_risk_winter), 
                          data = analysis_data_winter1)
#write_rds(er_results_int, paste0(file_path, "Output/er_results_int_age.rds"))
#summary(er_results_int)

int_emtrends <- emtrends(er_results_int, pairwise ~ chimney_clean_3level, var = "pm_mean_sampling_period_iqr")
...
er_results_lr <- glmer(lrti_events_di_winter ~ pm_mean_sampling_period_iqr + child_age + 
                          education_3level + residents_smoke + person_time_at_risk_winter +
                          (1 | home:cohort:area), 
                          data = analysis_data_winter1, family = binomial, nAGQ = 20)
write_rds(er_results_lr, paste0(file_path, "Output/er_results_lr.rds"))
...
er_results_lr <- read_rds(paste0(file_path, "Output/er_results_lr.rds"))
...
er_lr_tidy <- results_function(er_results_lr, "ER Log Reg")
...
# Combine cleaned results
er_results_combined <- rbind(er_tidy, er_lr_tidy, er_income_tidy, er_filter_comp_tidy, er_burn_level_tidy, 
...
er_plot_estimates <- er_results_combined %>%
  mutate(model = factor(model,
                        levels = c("ER Primary", "ER Log Reg", "ER + income", "ER + filt comp", "ER + burn level",
...
ER Primary = Negative binomial mixed model, as specified above  
ER Log Reg = Logistic regression instead of Negative binomial model framework  
