...
itt_results_int <<- glmer(lrti_events_di_total ~ treatment_assigned*int_var + 
                          ...
                          data = new_model_data, family = binomial, nAGQ = 20)
...
save_contrasts <<- confint(int_emmeans$contrasts) %>% 
  mutate(estimate = round(estimate, digits = 2),
         conf.low = round(asymp.LCL, digits = 2),
         conf.high = round(asymp.UCL, digits = 2)) %>% 
...
# Run primary model and print results
er_results_int <- glmer(lrti_events_di_winter ~ pm_mean_sampling_period_iqr*child_age + 
                          #child_age + 
                          education_3level + 
                          residents_smoke + 
...
int_emtrends <- emtrends(er_results_int, pairwise ~ child_age, var = "pm_mean_sampling_period_iqr")
...
save_contrasts <<- confint(int_emtrends$emtrends) %>% 
  mutate(estimate = round(pm_mean_sampling_period_iqr.trend, digits = 2),
         conf.low = round(asymp.LCL, digits = 2),
         conf.high = round(asymp.UCL, digits = 2)) %>% 
...
int_function(analysis_data_winter1, "child_age")
save_contrasts
pm_summary_table
