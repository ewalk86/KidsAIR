...
tidy_results <- tidy(itt_filter_type_results, conf.int = TRUE) %>% 
...
table(model.frame(itt_filter_type_results)$treatment_filter_type)
...
```{r, echo=TRUE}
# Run primary model and print results
itt_pm_results <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned + 
...
# Run primary model and print results
er_results_int <- glmer(lrti_events_di_winter ~ pm_mean_sampling_period_iqr*chimney_clean_3level + 
                          child_age + 
                          education_3level + 
                          residents_smoke + 
