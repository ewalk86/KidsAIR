...
# wood_collect_2level, burn_level_3level, residents_smoke

# Run primary model and print results
itt_pm_results_int <- glmer(log(pm_mean_sampling_period) ~ treatment_assigned*residents_smoke + 
                              child_age + 
                       (1 | cohort:area), data = analysis_data_winter1)
#write_rds(itt_pm_results_int, paste0(file_path, "Output/itt_pm_results_int_age.rds"))
#summary(itt_pm_results_int)

int_emmeans <- emmeans(itt_pm_results_int, pairwise ~ treatment_assigned | residents_smoke)
#summary(int_emmeans, conf.int = TRUE)
confint(int_emmeans)
