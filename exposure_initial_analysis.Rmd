...
# SUMs 
```{r}
#### Join PM dataset with full SUMs dataset
...
pm_sums_events_joined <- pm_home_mean_date %>% 
  left_join(sums_events_27, by = c("area", "home", "date_sums")) %>% 
  filter(sums_event == 1) %>% 
  arrange(area, home, date_sums) %>% 
  group_by(area, home, mean_pm) %>% 
  summarize(sums_events = n()) %>% 
  #mutate(sums_events = factor(sums_events))
  mutate(sums_events_cat = cut(sums_events, breaks = c(0, 1, 10),
                           labels = c("1", ">1")))

median(pm_sums_events_joined$sums_events)
