...
author: "Ethan Walker"
date: "Started 2 Feb 2020, Updated 26 March 2020"
output: pdf_document
---
...
pm_sums_results <- lmer(log(pm_mean_sampling_day) ~ temp_perc_25_2level + 
                              (1 | home), data = analysis_data)
...
# SUMs - events per sampling day - KEEP WORKING ON THIS
```{r}
sums_temp_change <- sums_rolling_pm_data %>% 
  ...
  mutate(sums_events = sum(heat_event, na.rm = TRUE),
         sums_events_cat = cut(sums_events, breaks = c(0, 5, 50),
                           labels = c("<4", "4+"),
                                 right = FALSE)) %>% 
  ungroup() %>% 
...
pm_sums_model <- lmer(mean_pm ~ sums_events + (1 | home), data = sums_temp_change)
pm_sums_model <- lmer(log(pm_mean_daily) ~ sums_events + (1 | home), data = sums_temp_change)
