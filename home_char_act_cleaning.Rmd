...
home_char_summary <- home_char_clean %>% 
  group_by(area) %>% 
  summarize(mean(home_sqft, na.rm = TRUE), 
            min(home_sqft, na.rm = TRUE), 
            median(home_sqft, na.rm = TRUE), 
            max(home_sqft, na.rm = TRUE))
home_char_summary
...
table_list <- home_char_clean %>% 
  #filter(area == "WMT") %>% 
  select(-home_winter_id, -home_sqft, -stove_model) 
lapply(table_list, table)

## Select specific var to assess individually/group by area
table(table_list$residents_total, table_list$area, useNA = "ifany")
```
...
# write_rds(home_act_clean, "Output/home_act_clean.rds")
```
...
table_list <- home_act_clean %>% 
  #filter(area == "WMT") %>% 
  select(-home_winter_id, -home_act_comments) 
lapply(table_list, table)
