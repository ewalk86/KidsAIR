...
author: "Ethan Walker"
date: "Started 23 Dec 2019, Updated 6 Jan 2020"
output: pdf_document
---
...
# Start here next:
## Make sure datetime_sums lines up with start/stop range
## Work on cleaning up temp_c_sums
## Ask about NN/AK finish dates and move on to repeating steps for those homes
write_rds(wmt_sums_filtered, "Output/wmt_sums_filtered.rds")

  

nn_sums_log <- read_rds("Output/nn_sums_data_log.rds")
# Need to get the NN medicalvisit file once data collection is finished
nn_sums_stop <- read_csv("Input/NN/nn_medicalvisit_20200107.csv") %>% 
  mutate(home_winter_id = HomeWinterID,
         final_visit_date = if_else(VisitDate == "11/4/2025", "11/4/2015", VisitDate),
         final_visit_date = mdy(final_visit_date)) %>% 
  group_by(home_winter_id) %>% 
...
ak_sums_log <- read_rds("Output/ak_sums_data_log.rds")
# Need to get the AK medicalvisit file once data collection is finished
ak_sums_stop <- read_csv("Input/AK/ak_medicalvisit_20200107.csv") %>% 
  mutate(home_winter_id = HomeWinterID,
         final_visit_date = if_else(VisitDate == "11/4/2025", "11/4/2015", VisitDate),
         final_visit_date = mdy(final_visit_date)) %>% 
  group_by(home_winter_id) %>% 
