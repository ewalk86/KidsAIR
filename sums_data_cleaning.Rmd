...
author: "Ethan Walker"
date: "Started 23 Dec 2019, Updated 14 Jan 2020"
output: pdf_document
---
...
wmt_sums_raw <- rbind(wmt_sums_csv, wmt_sums_csv_2, wmt_sums_csv_3,
  ...
  arrange(home, datetime_sums) %>% 
  distinct() %>% # remove duplicate lines that have same info from different files
  mutate(file_type = factor(file_type, levels = c("logtag", "temp_c",
                                                  "temp_f", "xlsx_c",
                                                  "xlsx_f")))

# This filtering will keep only the first instance when there are multiple
# observations for a specific datetime_sums
wmt_sums_filter <- wmt_sums_raw %>% 
  group_by(home) %>% 
  distinct(datetime_sums, .keep_all = TRUE) %>% 
  ungroup() %>% 
  arrange(home, datetime_sums)

# write_rds(wmt_sums_filter, "Output/wmt_sums_raw.rds")
```
...
# This filtering will keep only the first instance when there are multiple
# observations for a specific datetime_sums
nn_sums_filter <- nn_sums_raw %>% 
  group_by(home) %>% 
  distinct(datetime_sums, .keep_all = TRUE) %>% 
  ungroup() %>% 
  arrange(home, datetime_sums)

# write_rds(nn_sums_filter, "Output/nn_sums_raw.rds")
```
...
ak_sums_raw <- rbind(ak_sums_csv, ak_sums_csv_2, 
  ...
  arrange(home, datetime_sums) %>% 
  distinct() %>% 
  mutate(file_type = factor(file_type, levels = c("logtag", "temp_f",
                                                  "xlsx_c", "xlsx_f")))

# This filtering will keep only the first instance when there are multiple
# observations for a specific datetime_sums
ak_sums_filter <- ak_sums_raw %>% 
  group_by(home) %>% 
  distinct(datetime_sums, .keep_all = TRUE) %>% 
  ungroup() %>% 
  arrange(home, datetime_sums)

# write_rds(ak_sums_filter, "Output/ak_sums_raw.rds")
```
...
thresholds <- sums_clean %>% 
  group_by(area, home, winter_id) %>%
  mutate(temp_sums_40 = if_else(temp_c_sums >= 40, 1, 0),
         temp_sums_40_event = if_else(temp_sums_40 == 1 & lag(temp_sums_40) == 0, "Start", "Null"),
         temp_sums_40_event = if_else(is.na(temp_sums_40_event), "Start", temp_sums_40_event),
         temp_sums_40_event = if_else(temp_sums_40 == 1 & temp_sums_40_event == "Null",
                                      "Event", temp_sums_40_event),
         temp_sums_40_event = if_else(temp_sums_40_event == "Event" &
                                      lead(temp_sums_40_event) == "Null" &
                                      lead(temp_sums_40_event, 2) == "Null" &
                                      lead(temp_sums_40_event, 3) == "Null", "End", temp_sums_40_event)) %>% 
  group_by(area, home, winter_id, date_sums, temp_sums_40_event) %>% 
  summarize(n = n()) %>% 
  group_by(temp_sums_40_event) %>% 
  summarize(mean(n))


table(thresholds$temp_sums_40_event)
```
