...
lrti_final_data <- read_rds(paste0(file_path, "Output/lrti_final_data.rds")) %>% 
  ...
  ungroup() %>% 
  filter(person_time_at_risk_total > 0) %>% 
...
analysis_data <- health_exposure_data_sampling_day %>% 
  ...
  mutate(treatment = factor(treatment,
                            levels = c("Dearborn", "Smith", "Blackfoot")))
```
...
nb_results <- glm.nb(lrti_events ~ treatment + offset(log_person_time_at_risk), data = analysis_data)
odTest(nb_results)
