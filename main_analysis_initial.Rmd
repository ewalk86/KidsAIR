...
kids_kw_report_1obs <- read_rds(paste0(file_path, "Output/kids_kw_report_clean.rds")) %>% 
  group_by(home) %>% 
  mutate(mean_kw_per_exp = mean(kw_perc_expected, na.rm = TRUE)) %>% 
  ungroup() %>% 
  distinct(area, home, .keep_all = TRUE) %>% 
  mutate(kw_perc_exp_cat = cut(kw_perc_expected, 
                               breaks = c(0, 42, 83, 129, 50000),
                               labels = c("Q1", "Q2", "Q3", "Q4"))) %>% 
  dplyr::select(-home_winter_id, -treatment_assigned, -treatment_actual, -home_id)


kids_kw_report_2obs <- read_rds(paste0(file_path, "Output/kids_kw_report_clean.rds")) %>% 
  group_by(area, home_winter_id) %>% 
  mutate(mean_kw_per_exp = mean(kw_perc_expected, na.rm = TRUE)) %>% 
  ungroup() %>% 
  distinct(area, home_winter_id, .keep_all = TRUE) %>% 
  mutate(kw_perc_exp_cat = cut(kw_perc_expected, 
                               breaks = c(0, 37, 74, 118, 50000),
                               labels = c("Q1", "Q2", "Q3", "Q4"))) %>% 
  dplyr::select(-home_winter_id, -treatment_assigned, -treatment_actual, -home_id)
...
kids_analysis_data_1obs_per_child <- health_exposure_data_sampling_day %>% 
  ...
  left_join(kids_kw_report_1obs, by = c("area", "home")) %>% 
  distinct(area, child_id_num, .keep_all = TRUE) %>% 
  dplyr::select(area, home, cohort, child_id_num, child_age, lrti_events_total, 
         ...
         filter_type, mean_kw_per_exp, kw_perc_exp_cat) %>% 
  ...
  mutate(lrti_events_di_total = if_else(lrti_events_total == 0, 0, 1),
         filter_type2 = as.character(filter_type),
         filter_type2 = if_else(treatment_assigned == "Education", "Ed Home", filter_type2),
         filter_type2 = factor(filter_type2,
                              levels = c("Honeywell", "Large Filtrete", "Small Filtrete",
                                         "Winix", "Ed Home")),
         kw_perc_exp_cat2 = as.character(kw_perc_exp_cat),
         kw_perc_exp_cat2 = if_else(treatment_assigned == "Education", "Ed Home", kw_perc_exp_cat2),
         kw_perc_exp_cat2 = factor(kw_perc_exp_cat2,
                                  levels = c("Q1", "Q2", "Q3", "Q4", "Ed Home"))) %>% 
...
kids_analysis_data_2obs_per_child <- health_exposure_data_sampling_day %>% 
  ...
  left_join(kids_kw_report_1obs, by = c("area", "home", "winter_id")) %>% 
  distinct(area, child_id_num, winter_id, .keep_all = TRUE) %>% 
...
poisson_results <- glm(lrti_events_total ~ treatment + offset(log_person_time_at_risk), 
