...
kw_report_wmt <- read_csv(paste0(file_path, "Input/WMT/filtrete_report.csv"), 
  ...
  mutate(home_winter_id = as.factor(HomeWinterID),
         ...
         area = "WMT") %>% 
  dplyr::select(area, home, home_winter_id, kw_date, kw_hours, kw_notes, 
...
kw_summary <- kids_kw_report_clean %>% 
  filter(!is.na(kw_perc_expected)) %>% 
  #filter(area == "WMT") %>% 
  #group_by(treatment_assigned) %>% 
  summarize(mean(kw_perc_expected),
            sd(kw_perc_expected),
            n(),
            min(kw_perc_expected),
            median(kw_perc_expected),
            max(kw_perc_expected))
kw_summary
